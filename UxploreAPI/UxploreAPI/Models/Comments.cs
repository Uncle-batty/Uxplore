...
namespace UXplore.Models
{
    ...
    public static class CommentsEndpoints
    {
        public static void MapCommentsEndpoints(this IEndpointRouteBuilder routes)
        {
            ...
            // New endpoint to get comments by ListingID
            group.MapGet("/listing/{listingId}", async Task<Results<Ok<List<Comments>>, NotFound>> (int listingId, DataContext db) =>
            {
                var comments = await db.Comments.Where(c => c.Listing_ID == listingId).ToListAsync();
                return comments.Count > 0 ? TypedResults.Ok(comments) : TypedResults.NotFound();
            })
            .WithName("GetCommentsByListingId")
            .WithOpenApi();
        }
    }
}
